...
namespace locust
{
    ...
    void FieldCalculator::SetCavityFIRSample_norm(int bTE, int l, int m, int n, Kassiopeia::KSParticle& aFinalParticle, bool BypassTF)
    {
        ...
        fTFReceiverHandler->ConvolveWithComplexFIRFilterArray_norm(bTE, l, m, n, fJdotEBuffer, fTime);

        // t_off = ; // insure that there is a time step dt between function calls
        fTime = tTime; // update the time
    }

    std::pair<double,double> FieldCalculator::GetCavityFIRSample_norm(int bTE, int l, int m, int n)
    {
