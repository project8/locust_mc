/*
 * LMCPowerNormCavitySignalGenerator.cc
 *
 *  Created on: Jun 27, 2025
 *      Author: rmueller
 */

#include "LMCPowerNormCavitySignalGenerator.hh"
...
namespace locust
{
    LOGGER( lmclog, "PowerNormCavitySignalGenerator" );

    MT_REGISTER_GENERATOR(PowerNormCavitySignalGenerator, "power-norm-cavity-signal");

    PowerNormCavitySignalGenerator::PowerNormCavitySignalGenerator( const std::string& aName ) :
        Generator( aName ),
        fDoGenerateFunc( &PowerNormCavitySignalGenerator::DoGenerateTime ),
        fLO_Frequency( 0. ),
        fDeltaT( 0. ),
    ...
    PowerNormCavitySignalGenerator::~PowerNormCavitySignalGenerator()
    {
    ...
    bool PowerNormCavitySignalGenerator::ConfigureInterface( Signal* aSignal )
    {
        ...
        if( tParam.has( "transmitter" ))
        {
        	...
        	if (ntransmitters != 1)
        	{
        		LERROR(lmclog,"LMCPowerNormCavitySignalGenerator needs the kass-current transmitter.  Please choose transmitter:kass-current in the config file.");
                exit(-1);
        	}
        }
        else
    ...
    void PowerNormCavitySignalGenerator::SetParameters( const scarab::param_node& aParam )
    {
    	fParam = &aParam;
    }


    const scarab::param_node* PowerNormCavitySignalGenerator::GetParameters()
    {
    	return fParam;
    }


    bool PowerNormCavitySignalGenerator::Configure( const scarab::param_node& aParam )
    {
    ...
    bool PowerNormCavitySignalGenerator::RecordRunParameters( Signal* aSignal )
    {
#ifdef ROOT_FOUND
    ...
    bool PowerNormCavitySignalGenerator::SetSeed(int aSeed)
    {
    ...
    int PowerNormCavitySignalGenerator::GetSeed()
    {
        return fTrackDelaySeed;
    }

    bool PowerNormCavitySignalGenerator::RandomizeStartDelay()
    {
        scarab::param_node default_setting;
        default_setting.add("name","uniform");
    ...
    bool PowerNormCavitySignalGenerator::CrossCheckAliasing( Signal* aSignal, double dopplerFrequency )
    {
    ...
    bool PowerNormCavitySignalGenerator::CrossCheckCavityConfig()
	{
    ...
    void PowerNormCavitySignalGenerator::Accept( GeneratorVisitor* aVisitor ) const
    {
        aVisitor->Visit( this );
        return;
    }

    Signal::State PowerNormCavitySignalGenerator::GetDomain() const
    {
        return fRequiredSignalState;
    }

    void PowerNormCavitySignalGenerator::SetDomain( Signal::State aDomain )
    {
        ...
        {
            fDoGenerateFunc = &PowerNormCavitySignalGenerator::DoGenerateFreq;
        }
        else
        {
    ...
    bool PowerNormCavitySignalGenerator::DriveMode(Signal* aSignal, unsigned index)
    {
    ...
    void PowerNormCavitySignalGenerator::KassiopeiaInit(const std::string &aFile)
    {
    ...
    void PowerNormCavitySignalGenerator::WakeBeforeEvent()
    {
        fInterface->fPreEventCondition.notify_one();
        return;
    }

    bool PowerNormCavitySignalGenerator::TryWakeAgain()
    {
    	int count = 0;
    	while (count < 10)
    	{
     ...
     bool PowerNormCavitySignalGenerator::ReceivedKassReady()
    {
    ...
    bool PowerNormCavitySignalGenerator::DoGenerate( Signal* aSignal )
    {
        return (this->*fDoGenerateFunc)( aSignal );
    }


    bool PowerNormCavitySignalGenerator::DoGenerateFreq( Signal* aSignal )
    {
        return true;
    }

    bool PowerNormCavitySignalGenerator::DoGenerateTime( Signal* aSignal )
    {
        ...
        if (fInterface->fTransmitter->IsKassiopeia())
        {
            ...
            std::thread tKassiopeia (&PowerNormCavitySignalGenerator::KassiopeiaInit, this, gxml_filename); // spawn new thread
