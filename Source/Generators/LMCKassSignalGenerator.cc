...
//        printf("GroupVelocity is %g, tCutOffFreq is %g, 2PIfcyc is %g\n", tGroupVelocity, tCutOffFrequency, 2.*KConst::Pi()*tCyclotronFrequency); getchar();
        ...
        {
            // initialize phases.
            phi_t1 = 2.*KConst::Pi()*(tCenterToAntennaDistance - tPositionZ) / (tGroupVelocity / tDopplerFrequencyAntenna);
            phi_t2 = 2.*KConst::Pi()*(tPositionZ + 2.*tCenterToShortDistance + tCenterToAntennaDistance) / (tGroupVelocity / tDopplerFrequencyShort);
