...
//    	printf("tArea is %g\n", tArea); getchar();
    ...
    std::vector<double> RectangularWaveguide::TE_E(int m, int n, double xKass, double yKass) const
    {
    ...
    std::vector<double> RectangularWaveguide::TE_H(int m, int n, double xKass, double yKass) const
    {
    	double x = xKass + fInterface->fX/2.;
    	double y = yKass + fInterface->fY/2.;
    	...
    	double kc = pow(k1*k1+k2*k2,0.5);
    	double omega = LMCConst::C()*kc;  // TO-DO:  Make this frequency-dependent.
    	double tHx = kc*m*LMCConst::Pi()/kc/kc/fInterface->fX * sin(k1*x) * cos(k2*y);
    	double tHy = kc*n*LMCConst::Pi()/kc/kc/fInterface->fY * cos(k1*x) * sin(k2*y);
    	TE_H.push_back(tHx);
    	TE_H.push_back(tHy);
        return TE_H;
    }

    std::vector<double> RectangularWaveguide::TM_E(int m, int n, double xKass, double yKass) const
    {
    	double x = xKass + fInterface->fX/2.;
    	double y = yKass + fInterface->fY/2.;
    	...
    	double kc = pow(k1*k1+k2*k2,0.5);
    	double omega = LMCConst::C()*kc;  // TO-DO:  Make this frequency-dependent.
    	double tEx = kc*m*LMCConst::Pi()/kc/kc/fInterface->fX * cos(k1*x) * sin(k2*y);
    	double tEy = kc*n*LMCConst::Pi()/kc/kc/fInterface->fY * sin(k1*x) * cos(k2*y);
    	TM_E.push_back(tEx);
    	TM_E.push_back(tEy);
        return TM_E;
    }

    std::vector<double> RectangularWaveguide::TM_H(int m, int n, double xKass, double yKass) const
    {
    	double x = xKass + fInterface->fX/2.;
    	double y = yKass + fInterface->fY/2.;
